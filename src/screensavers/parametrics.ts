...
export function createParametrics() {
  return createShaderToyScreensaver('Parametrics', 'by nimitz', `
#define time iTime
...
// Procedural noise to replace iChannel0 texture
float hash12(vec2 p)
{
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    col *= (noise(p*0.25*iResolution.x+fract(time)*vec2(51,100))-0.5)*.24+1.;
    col = mix(col,tone(col,1.7),0.5);

    #elif COLOR_TYPE == 2
    vec3 col = vec3(5.,1.5,.5)*log(rz+1.05);
    col = tone(col,8.5);
    col *= (noise(p*0.17*iResolution.x+fract(time)*vec2(51,100))-0.5)*.12+1.;

    #else
    rz = pow(rz*50.,.7);
    vec3 col = vec3(.6,.2,1.)/(rz+1.5)*9.;
    col *= (noise(p*0.25*iResolution.x+fract(time)*vec2(51,100))-0.5)*.45+1.;
    col = tone(col,1.6);
    #endif
